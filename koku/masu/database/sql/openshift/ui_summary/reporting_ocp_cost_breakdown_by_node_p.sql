DELETE FROM {{schema | sqlsafe}}.reporting_ocp_cost_breakdown_by_node_p
WHERE usage_start >= {{start_date}}::date
    AND usage_start <= {{end_date}}::date
    AND source_uuid = {{source_uuid}}
;

INSERT INTO {{schema | sqlsafe}}.reporting_ocp_cost_breakdown_by_node_p (
    id,
    cluster_id,
    cluster_alias,
    node,
    ...
    WHERE usage_start >= {{start_date}}::date
        AND usage_start <= {{end_date}}::date
        AND source_uuid = {{source_uuid}}
    GROUP BY usage_start, cluster_id, cluster_alias, node, cost_model_rate_type, cost_model_rate_name
;
