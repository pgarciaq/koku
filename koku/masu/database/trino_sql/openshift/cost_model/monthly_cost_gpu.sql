...
WITH cte_unutilized_uptime_hours AS (
    select
        ...
        regexp_replace(json_extract_scalar(node_ut.node_labels, '$.nvidia_com_gpu_product'), '[^a-zA-Z0-9]+', ' ') as model,
    ...
    group by node_ut.node, regexp_replace(json_extract_scalar(node_labels, '$.nvidia_com_gpu_product'), '[^a-zA-Z0-9]+', ' '), DATE(node_ut.interval_start)
)
SELECT
    uuid() as uuid,
    {{report_period_id}} as report_period_id,
