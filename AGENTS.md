...
**ROS optimizations UI:** New recommendation list tabs must include term/engine
dropdowns (`OptimizationsProjectionToolbar`) and inherit projection on detail pages.
See `koku-ui/apps/koku-ui-ros/AGENTS.md` and workspace rule `koku-ecosystem.mdc`.

## Building and Deploying koku-ui (On-Prem Container Image)

### Containerfile

The **only** Containerfile to use is `apps/koku-ui-onprem/Containerfile` (in the
koku-ui repo). It is a two-stage build:

1. **Builder** (`ubi10/nodejs-22-minimal`): Runs `npm ci`, then builds each workspace
   with `build:onprem` (HCCM, ROS, Sources) or `build` (onprem shell).
2. **Runtime** (`ubi10/nginx-126`): Copies each app's `dist/` into a named directory
   under `/opt/app-root/src/`, serves on port 8080.

**Never use** `.archive/Dockerfile` (CentOS 7 / Node 8 / yarn — obsolete).
**Never create** a new Containerfile — use the existing one.

### Image Architecture (Module Federation)

The on-prem UI is a Webpack Module Federation setup with four bundles served by
a single nginx container:

| Directory | App | publicPath | Build command |
|-----------|-----|------------|---------------|
| `onprem/` | Host shell | `/` | `npm run build --workspace=@koku-ui/koku-ui-onprem` |
| `costManagement/` | HCCM (federated) | `/costManagement/` | `npm run build:onprem --workspace=@koku-ui/koku-ui-hccm` |
| `costManagementRos/` | ROS (federated) | `/costManagementRos/` | `npm run build:onprem --workspace=@koku-ui/koku-ui-ros` |
| `sources/` | Sources (federated) | `/sources/` | `npm run build:onprem --workspace=@koku-ui/koku-ui-sources` |

Each federated module **must** have a `plugin-manifest.json` in its `dist/` output.
This file is only generated by `build:onprem`. Using `build` or `fec build` generates
cloud-only `fed-mods.json` with wrong `baseURL` paths — the UI will fail at runtime
with `JSON.parse: unexpected character` errors.

### Check architecture first (required before any image build)

**Do not assume SNO or a cluster name (e.g. UXSNO) implies aarch64.** Lab clusters
mix architectures. Image architecture must match the **cluster nodes**, not the
hypervisor name alone.

```bash
# Build host (where podman/docker runs)
uname -m

# Target cluster (after oc login)
oc get nodes -o custom-columns=NAME:.metadata.name,ARCH:.status.nodeInfo.architecture
```

| Cluster `ARCH` | Build on matching host | Cross-build (slower; use only if needed) |
|----------------|------------------------|-------------------------------------------|
| `amd64` | Native on x86_64 — **no** `--platform` | — |
| `arm64` / `aarch64` | Native on aarch64 — **no** `--platform` | `podman build --platform linux/arm64` from x86_64 |

**Examples:** UXSNO (`uxsno-sno.karmalabs.corp`) is **amd64** (Dell R640). Apollo SNO
is **aarch64**. Only follow
[`aarch64-sno-deployment.mdc`](../cost-onprem-chart/.cursor/rules/aarch64-sno-deployment.mdc)
when the cluster reports `arm64`/`aarch64`.

Build on the cluster's architecture **before** push/deploy. Wrong-arch images cause
`CrashLoopBackOff`, silent `IfNotPresent` stale tags, or extremely slow QEMU builds.

### Build, Push, Deploy (UXSNO)

```bash
# 1. Build (from koku-ui repo root — file is Containerfile, NOT Dockerfile)
cd ~/dev/koku/koku-ui
TAG="phase17-$(date -u +%Y%m%d%H%M)"
podman build -t koku-ui:$TAG -f apps/koku-ui-onprem/Containerfile .

# 2. Push to OpenShift registry
REGISTRY="default-route-openshift-image-registry.apps.uxsno.karmalabs.corp:443/cost-onprem"
...
podman tag koku-ui:$TAG $REGISTRY/koku-ui:$TAG
podman push --tls-verify=false $REGISTRY/koku-ui:$TAG

# If podman push fails on blob-reuse auth ("unable to retrieve auth token"), use skopeo instead:
TOKEN=$(oc create token image-pusher -n cost-onprem --duration=1h)
skopeo copy --dest-tls-verify=false --dest-creds "image-pusher:${TOKEN}" \
  containers-storage:localhost/koku-ui:${TAG} \
  docker://${REGISTRY}/koku-ui:${TAG}

# 3. Deploy (container name is "app", NOT "ui" or "nginx")
INTERNAL="image-registry.openshift-image-registry.svc:5000/cost-onprem"
oc set image deployment/cost-onprem-ui app=$INTERNAL/koku-ui:$TAG -n cost-onprem
oc rollout status deployment/cost-onprem-ui -n cost-onprem --timeout=120s
```

### Critical Rules

- **Always use unique tags** (e.g., `phase17-202606220623`). `imagePullPolicy: IfNotPresent`
  means reusing a tag silently keeps the old image.
- **Container name is `app`**. The `cost-onprem-ui` deployment has two containers:
  `oauth-proxy` and `app`. Use `app` with `oc set image`.
- **Build on hypervisor if local push fails**. The hypervisor always has working `oc`
  credentials:
  ```bash
  ssh root@dell-r640-041.bkr.lab.eng.rdu2.dc.redhat.com
  cd /root/koku-ui && git pull
  podman build -t koku-ui:$TAG -f apps/koku-ui-onprem/Containerfile .
  ```
- **skopeo fallback for registry push:** When `podman push` fails reusing blobs with
  `unable to retrieve auth token`, copy with `skopeo copy containers-storage:localhost/<image>:<tag>
  docker://<registry>/<image>:<tag>` and `--dest-creds "image-pusher:$TOKEN"`.
...
**First step on every deploy** — verify build host and cluster (see koku-ui
[Check architecture first](#check-architecture-first-required-before-any-image-build)
and cost-onprem
[`aarch64-sno-deployment.mdc` §0](../cost-onprem-chart/.cursor/rules/aarch64-sno-deployment.mdc)):
...
- **amd64 cluster (e.g. UXSNO):** build natively on x86_64; do **not** pass
  `--platform linux/arm64`.
- **aarch64 cluster (e.g. Apollo SNO):** build on aarch64 or use
  `--platform linux/arm64`; see aarch64-specific rules below.

SNO ≠ aarch64. Do not apply aarch64-only guidance to x86_64 clusters.

## aarch64-only deployment rules

The following apply **only** when the cluster nodes report `arm64`/`aarch64`.
...
## aarch64 Images (arm64 clusters only)

Six images must be built for **aarch64** clusters (not needed on amd64 UXSNO):
koku, ros-ocp-backend, insights-ingress-go, autotune (Kruize), postgresql:16,
valkey. Build all of them proactively before attempting any Helm install on arm64.
